...
export interface CollectionBackend<T> {
  name: string
  index: Map<string, T>
...
export class CollectionRepositoryBackend<T> {
  ...
  constructor(
    ...
    readonly options: CollectionRepositoryBackendOptions<T>,
  ) {
...
export class CollectionLoaderBackend<T> {
  ...
  constructor(
    readonly name: string,
    readonly options: CollectionLoaderBackendOptions<T>,
  ) {
    this.load = makeCachedLoader2({
      name: this.name,
      index: this.index,
...
export class Collection<T> {
  ...
  constructor(public options: CollectionOptions<T>) {
