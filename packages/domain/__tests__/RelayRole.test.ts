...
import {RelayRole, RelayRoleBuilder} from "../src/kinds/RelayRole"
...
describe("RelayRole", () => {
  it("reads the role id, label, description, color, and order", async () => {
    const reader = await RelayRole.fromEvent(
      makeEvent({
        tags: [
          ["d", "moderator"],
          ["label", "Moderator"],
  ...
  it("returns empty color components when there is no color tag", async () => {
    const reader = await RelayRole.fromEvent(makeEvent({tags: [["d", "moderator"]]}))

    expect(reader.color()).toEqual({hue: "", saturation: "", lightness: ""})
  })

  it("defaults order to zero when missing or invalid", async () => {
    const missing = await RelayRole.fromEvent(makeEvent({tags: [["d", "moderator"]]}))
    const invalid = await RelayRole.fromEvent(
      makeEvent({
        tags: [
          ["d", "moderator"],
          ["order", "abc"],
  ...
  it("builds a role from scratch", async () => {
    const tmpl = await new RelayRoleBuilder()
  ...
  it("round-trips an existing role without duplicating tags", async () => {
    ...
    const tmpl = await reader.builder().setLabel("Mod").setOrder(3).toTemplate()
  ...
  it("requires a d tag", async () => {
    await expect(new RelayRoleBuilder().setLabel("Moderator").toTemplate()).rejects.toThrow(/d tag/)
  })

  it("throws on the wrong kind", async () => {
    await expect(RelayRole.fromEvent(makeEvent({kind: NOTE}))).rejects.toThrow()
  })
})
