...
- Writers modify tags directly in most cases. Only override `renderDomainTags` for derived tags (e.g. `TimeEvent`'s `D` day-buckets) — the base class's `renderBehaviorTags` already covers `h`/`d`/`-`/`expiration`/`content-warning`.
